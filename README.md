...
- **Local Dev:** Docker & Docker Compose (`compose.yml`), Nginx
- **Production Hosting:** Zeabur natively (via `zeabur.json`)
...
3. **Start the Docker Containers**
   Run the following command to build the images and start the services (Laravel App, Nginx, PostgreSQL) in the background:
   ```bash
...
5. **Wait for Build & Deployment**
   - Zeabur will read the `zeabur.json` file in this repository.
   - It will automatically run `composer install`, cache your configuration, and run `php artisan migrate --force` as specified in the configuration.
...
- **Nginx Gzip & Caching**: The `docker/nginx/nginx.conf` file includes `gzip` compression configurations and long-term browser caching rules for static assets.
- **Database Security**: The `compose.yml` ensures that your database port `5432` is securely bound to local interfaces (`127.0.0.1`) mitigating the risk of exposure to the public internet on VMs.
